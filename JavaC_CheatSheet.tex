%%%%%%%%%%%%%%%%%%%%%%%%%
% Dokumentinformationen %
%%%%%%%%%%%%%%%%%%%%%%%%%
\title{JavaC - CheatSheet}
\author{\href{mailto:jan.brupbacher@hsr.ch}{J. Brupbacher}}
\newcommand{\authoremail}{\href{mailto:jan.brupbacher@hsr.ch}{jan.brupbacher@hsr.ch} }
\newcommand{\versioninfo}{$ V1.1 $}

\documentclass[10pt,twoside,a4paper,fleqn]{scrartcl}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Standard Header für 
% - Makros 
% - Farben
% - Mathematische Operatoren
\include{header/zusammenfassung}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Ergänzungen für Package kommen hier hin:
%\usepackage{hyperref}

\usepackage[table]{xcolor}
\renewcommand{\familydefault}{\sfdefault}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}
\lstset{style=Java} 
\textbf{\huge{JavaC - CheatSheet}}\\
\input{sections/Allgemein}
\input{sections/Collections}
\input{sections/Vererbung}
\input{sections/Schnittstellen}
\input{sections/SpezielleGrundfunktionen}
\input{sections/Generics}
\input{sections/Exceptions}
\input{sections/Lambdas}
\input{sections/StreamAPI}
\input{sections/InputOutput}
\input{sections/Enums}
\end{document}
